...
class Generator(object):
    ...
    def __init__(self,x,y,translate=[0,0],flip=[0,0],noise=[0,0]):
    ...
    def __aug_data(self):
        ...
        if self.shift_width != 0 or self.shift_height !=0:
            ...
            self.y_aug = np.vstack((self.y_aug,np.roll(self.y,shift=(self.shift_height,self.shift_width),axis=(1,2))))

        # Flip values
        if self.curr_flip != 'none':
            if self.curr_flip == 'horizontal':
                self.x_aug = np.vstack((self.x_aug,np.flip(self.x,axis=1)))
                self.y_aug = np.vstack((self.y_aug,np.flip(self.y,axis=1)))
            elif self.curr_flip == 'vertical':
                self.x_aug = np.vstack((self.x_aug,np.flip(self.x,axis=2)))
                self.y_aug = np.vstack((self.y_aug,np.flip(self.y,axis=2)))
            else:
                flipped_x = np.flip(self.x,axis=1)
                flipped_y = np.flip(self.y,axis=1)
                self.x_aug = np.vstack((self.x_aug,np.flip(flipped_x,axis=2)))
                self.y_aug = np.vstack((self.y_aug,np.flip(flipped_y,axis=2)))

        # Rotate by random value DONT USE FOR THIS
        if self.curr_rot != 0:
            self.x_aug = np.vstack((self.x_aug,rotate(self.x,self.curr_rot,axes=(1,2),reshape=False)))
            self.y_aug = np.vstack((self.y_aug,self.y))

        # Add noise to values
        if self.curr_noise[0] != 0 or self.curr_noise[1] != 0:
            self.x_aug = np.vstack((self.x_aug,self.x+np.random.normal(0,self.curr_noise[0],self.x.shape)))
            self.y_aug = np.vstack((self.y_aug,self.y+np.random.normal(0,self.curr_noise[1],self.y.shape)))
